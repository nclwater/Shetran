...
USE ALmod,    ONLY : ALINIT, ALINTP, ALCHK, ALCHKI
...
!
! Added by SB
INTEGER :: NCATEE, NTABEE  
INTEGER :: NCAT (NCONEE), NTAB (NOCTAB, NCONEE)  
INTEGER :: NCATTY (NELEE, NCONEE)  
DOUBLEPRECISION CTAB (NOCTAB, NOCTAB, NCONEE)  
DOUBLEPRECISION DTAB (NOCTAB, NOCTAB, NCONEE)  
...
CALL CMRD (CMD, CMP, NCATEE, NCONEE, NELEE, total_no_elements, total_no_links, NLFEE, NSEE, &  !"JE"
 NS, NSEDEE, NSED, NTABEE, NX, NXEE, NYEE, NY, NLYRBT (total_no_links + 1, 1), &  !"JE"
 ICMXY, ICMBK, ICMREF (1, 5), BEXBK, LINKNS, NCAT, NCATTY, NCON, &  !"JE"
 NCOLMB (total_no_links + 1), NTAB, DBS, DBDI, CCAPI, CCAPE, CCAPR, CCAPB, &  !"JE"
 CTAB, DTAB, IIICF, SOFN, GNN, GGLMSO, ALPHBD, ALPHBS, KDDLS, &  !"JE"
...
CALL MUERR2 (CMP, total_no_elements, NELEE, total_no_links, NCATEE, NTABEE, NCON, NCONEE, &
 NCAT, NTAB, NCATTY, ISCNSV, CTAB, DTAB, LDUM1)
!----------------------------------------------------------------------*
...
!     between values in the depth/conc. tables
      CALL ALINTP (LLEE, NCETOP, total_no_elements, NELEE, total_no_links, NCAT (NCONT), &
       NCATEE, NTABEE, NCATTY (total_no_links + 1, NCONT), NCOLMB (total_no_links + 1), &
       NTAB (1, NCONT), CTAB (1, 1, NCONT), DTAB (1, 1, NCONT), &
...
!SSSSSS SUBROUTINE MUERR2 (CPR, NEL, NELEE, NLF, NCATEE, NTABEE, NCON, &
SUBROUTINE MUERR2 (CPR, total_no_elements, NELEE, total_no_links, NCATEE, NTABEE, NCON, &
 NCONEE, NCAT, NTAB, NCATTY, ISCNSV, CTAB, DTAB, LDUM)
!
!--------------------------------------------------------------------*
!
...
!
      DO 160 NELMTY = 1, NCAT (I)  
         CALL ALCHK(EEERR, 2104, CPR, NELMTY, NELMTY, 1, IUNDEF, &
          'DTAB[ncat,1]', 'EQ', ZERO1, ZERO , DTAB (NELMTY, 1, &
          I) , NERR, LDUM)
         DO 170 NTBL = 2, NTAB (NELMTY, I)  
            PREVDP = DTAB (NELMTY, NTBL - 1, I)  
            CALL ALCHK(EEERR, 2105, CPR, NELMTY, NELMTY, NTBL, &
             IUNDEF, 'DTAB[ncat,ntab]', 'GT', (/PREVDP/) , &
             ZERO , DTAB (NELMTY, NTBL, I) , NERR, LDUM)
  170          END DO  
  160       END DO  
!
!       *CTAB
...
!
      DO 260 NELMTY = 1, NCAT (I)  
         DO 270 NTBL = 1, NTAB (NELMTY, I)  
            CALL ALCHK(EEERR, 2106, CPR, NELMTY, NELMTY, NTBL, &
             IUNDEF, 'CTAB[nmne,ntab]', 'GE', zero1, zero , &
             CTAB (NELMTY, NTBL, I) , NERR, LDUM)
  270          END DO  
