...
!> values from the sediment/contaminant interface in [[sy_state]] so that
!> [[cm_column:COLM]] and [[cm_sorption:FRET]] can form old-state storage, erosion, and
...
!> |:------|:--------------------|:---------|
!> | `DLSO`, `GNUO` | [[cm_input:INCM]]; shifted by [[cm_column:COLMW]]. | Previous values in `COLM`. |
!> | `FBETAO`, column `FDELO` | No startup seed; shifted by [[cm_column:COLMSM]]. | Column `FRET` calls. |
!> | `FBBEDO`, link `FDELO`, `FBTSDO` | `INCM`; shifted by [[cm_channel:LINKSM]]. | Link `FRET` calls. |
!>
...
!> is held in `DSWO` in [[cm_column_previous]]. This module supplies no declaration
...
!> `INCM` initializes `FBTSDO` from `FBTSD`; as recorded in [[sy_state]], no
