...
!> | 11 | Polynomial channel boundary. |
   SUBROUTINE JEOCBC(IXER, NOCBC)
      ...
      INTEGER, DIMENSION(NELEE), SAVE :: IDUM !! Integer input workspace; filled by `AREADI` over `1..total_no_elements` and read by element number, so `NELEE` bounds it. `SAVE` keeps it in static storage, as the former module variable was.
...
!> @endhistory
   SUBROUTINE OCREAD(KONT, TDC, TFC, CATR, DDUM2)
      ...
      INTEGER, DIMENSION(NELEE), SAVE :: IDUM !! Integer input workspace; filled by `AREADI` over `1..total_no_elements`, and passed to `OCPLF` as `NXDEF(NOCTAB)`, so `NELEE` bounds it. `SAVE` keeps it in static storage, as the former module variable was.
