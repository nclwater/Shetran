...
! Created as part of FRmod refactoring
   ...
   USE SGLOBAL, ONLY : NELEE, nlfee, noctab, NXEE, NYEE, NVEE, BDEVER, SHEVER, BANNER, FILNAM, DIRQQ, &
      ...
      FFFATAL, zero, PPPRI, half, one, two, ione, cellarea, &
      total_no_elements, total_no_links, top_cell_no, DXQQ, DYQQ, ZGRUND, NSEDEE, ERROR
   USE CONT_CC, ONLY :    CCAPE, CCAPR, CCAPB, GNN, alphbd, alphbs, alpha, fads, &
      CCAPI, CCAPIO, IIICF, IIICFO, CCCC, CCCCO, SSSS, SSSSO, &
      GCPLA, GGLMSO, CCAPIN, KDDLS, KDDSOL, NCON
   ...
   USE framework_component_initialization, ONLY : MUERR2, INPL
   USE framework_spatial_setup, ONLY : FRIND

   IMPLICIT NONE

   ! Original FRmod module variables
   INTEGER :: IAOUT
   ...
   CHARACTER(256) :: msg
   INTEGER :: hour_now
   DOUBLEPRECISION :: uznowt, qoctotextra(100) = 0.0d0
   DOUBLEPRECISION :: PREVTM
   DOUBLEPRECISION :: TIMB = 0.0d0
   ...
   DOUBLEPRECISION :: qoctot = 0.0d0, uzold = 0.0d0
   INTEGER :: next_hour = 1, icounter2 = 0

   ! Export all shared functionality
   PUBLIC :: INFR, DINET, DINOC, INET, INSM, INBK, INCM
   PUBLIC :: FRDIM
   PUBLIC :: TITLE, TSH, TCH, BSOFT, BSTORE, qoctot, uzold, next_hour, icounter2
   PUBLIC :: IAOUT, ALLOUT, DTAO, BFRTS1, BFRTS2, BINFRP, BTIME
   PUBLIC :: BPPNET, BPEPOT, BPQOC, BPDEP, BPQF, BPQH, BPQSZ, BPHSZ, BPBAL, BPSD
   PUBLIC :: msg, hour_now, uznowt, qoctotextra, PREVTM, TIMB, FIRST_frmb, SEDSRT, GNUCUM, DLSSRT
...
!:ET7
         READ (ETD, 100) ET_HEAD
100      FORMAT   (A)
         IF (BINETP) WRITE(PPPRI, 110) ET_HEAD
110      FORMAT   ('0'//1X, A)
!-------------------------------------
!  READ PARAMETER DATA
!-------------------------------------
...
!SSSSSS SUBROUTINE FRDIM (BINFRP_LOCAL)
   SUBROUTINE FRDIM (BINFRP_LOCAL)
!----------------------------------------------------------------------*
!
...
!
! Input arguments
      LOGICAL, INTENT(IN) :: BINFRP_LOCAL
!
! Locals, etc
...
!
               ...
               IF (INEXT1.GT.0) THEN
                  ...
                  ENDIF
                  IF (I1.EQ.5.OR.I1.EQ.7) THEN
                     DXQQ (IEL) = DXQQ (IEL) - DIFF
                  ENDIF
                  IF (I1.EQ.6.OR.I1.EQ.8) THEN
                     DYQQ (IEL) = DYQQ (IEL) - DIFF
                  ENDIF
               ENDIF
!
