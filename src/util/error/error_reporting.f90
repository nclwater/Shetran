!> summary: The numbered diagnostic reporter, the error counters and program termination.
...
!> diagnostic reporter [[RAISE_ERROR]], the termination routine [[ERR_STOP]],
!> the severity selectors passed to them, and the per-code occurrence counters.
!> It is the module the whole tree reports through, so it is kept as small as
!> the reporting itself: the `IOSTAT`/`STAT` wrappers that most callers
!> actually use live next door in [[error_status]], which calls into this
!> module and is never called back.
!>
!> @note
!> The reporting interface is unchanged from its former `mod_error` and
!> `sglobal` forms, so existing call sites continue to work.
!> @endnote
!>
...
CONTAINS
   ...
   !> | 2026-08-31 | SvB | Moved from the former `sglobal` to the former `mod_error` and renamed the selectors and counters. |
   !> @endhistory
