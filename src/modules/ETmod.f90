...
!> @endhistory
   SUBROUTINE INITIALISE_ETMOD()
      ...
      CALL errstat_alloc(ios, "RA",location)
      ALLOCATE (RC(NV), STAT=ios)
      CALL errstat_alloc(ios, "RC",location)
      ALLOCATE (RTOP(NV), STAT=ios)
      CALL errstat_alloc(ios, "RTOP",location)
      ALLOCATE (CSTCAP(NV), STAT=ios)
      CALL errstat_alloc(ios, "CSTCAP",location)
      ALLOCATE (CK(NV), STAT=ios)
      CALL errstat_alloc(ios, "CK",location)
      ALLOCATE (CB(NV), STAT=ios)
      CALL errstat_alloc(ios, "CB",location)
      ALLOCATE (DEL(MAX(NV, NM, NRAIN)), STAT=ios)
      CALL errstat_alloc(ios, "DEL",location)
      ALLOCATE (PSI4(LLEE), STAT=ios)
      CALL errstat_alloc(ios, "PSI4",location)
      ALLOCATE (UZALFA(LLEE), STAT=ios)
      CALL errstat_alloc(ios, "UZALFA",location)
      ALLOCATE (CSTCA1(NV), STAT=ios)
      CALL errstat_alloc(ios, "CSTCA1",location)
      ALLOCATE (PLAI1(NV), STAT=ios)
      CALL errstat_alloc(ios, "PLAI1",location)
      ALLOCATE (CLAI1(NV), STAT=ios)
      CALL errstat_alloc(ios, "CLAI1",location)
      ALLOCATE (VHT1(NV), STAT=ios)
      CALL errstat_alloc(ios, "VHT1",location)
      ALLOCATE (PS1(NV,NUZTAB), STAT=ios)
      CALL errstat_alloc(ios, "PS1",location)
      ALLOCATE (FET(NV,NUZTAB), STAT=ios)
      CALL errstat_alloc(ios, "FET",location)
      ALLOCATE (RCF(NV,NUZTAB), STAT=ios)
      CALL errstat_alloc(ios, "RCF",location)
      ALLOCATE (RELCST(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "RELCST",location)
      ALLOCATE (TIMCST(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "TIMCST",location)
      ALLOCATE (RELPLA(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "RELPLA",location)
      ALLOCATE (TIMPLA(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "TIMPLA",location)
      ALLOCATE (RELCLA(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "RELCLA",location)
      ALLOCATE (TIMCLA(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "TIMCLA",location)
      ALLOCATE (RELVHT(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "RELVHT",location)
      ALLOCATE (TIMVHT(NV,NVBP), STAT=ios)
      CALL errstat_alloc(ios, "TIMVHT",location)
