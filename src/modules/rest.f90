...
!USE SGLOBAL,    ONLY : NELEE, NVEE
   ...
   USE AL_D,    ONLY :  flerror_counter, balanc, syerror_counter, cmerror_counter, nstep, carea, DTMET2, BHOTRD, &
   ...
   use mod_error, ONLY : ERROR, ERRLVL_fatal, ERRLVL_warn, FID_logfile, ALSTOP
...
!SSSSSS SUBROUTINE extra_output
   SUBROUTINE extra_output()
      ...
      DO I = 0, 100
         IF (FLerror_counter (I) .GT.0) WRITE(FID_logfile, 1500) I + 1000, FLerror_counter (I)
      END DO
      DO I = 0, 100
         IF (SYerror_counter (I) .GT.0) WRITE(FID_logfile, 1500) I + 2000, SYerror_counter (I)
      END DO
      DO I = 0, 100
         IF (CMerror_counter (I) .GT.0) WRITE(FID_logfile, 1500) I + 3000, CMerror_counter (I)
      END DO
...
9900  FORMAT ('Normal completion of SHETRAN run: ',F10.2, ' hours, ', &
   ...
   SUBROUTINE METIN (IFLAG)
      ...
      ! BMETAL, BMETDATES, prd, NRAIN, FID_logfile, uznow, PINP, ZERO, dtmet2, MELAST,
               ...
               pet_read_loop: DO
                  ...
                  IF (BMETDATES) THEN
                     ...
                     ELSE
                        ...
                        READ (tmp, *, IOSTAT=ios) PEIN(1:NM)
                        IF (ios > 0) then 
                           WRITE (*, 9022) ' Error parsing PET values from line.'
                           CALL ALSTOP(255)
                        END IF
                     END IF
                     ...
                     IF (ISTA) THEN
                        ...
                        IF (ios == 0) THEN
                           READ (tmp, *, IOSTAT=ios) TAHIGH(1:NM)
                           IF (ios > 0) then 
                              WRITE (*, 9022) ' Error parsing max temp values from line.'
                              CALL ALSTOP(255)
                           END IF
                           tahdate = HOUR_FROM_DATE(tahyear, tahmonth, tahday, tahhour, tahminute)
                        END IF
                     ...
                     IF (ISTA) THEN
                        ...
                        IF (ios == 0) THEN
                           READ (tmp, *, IOSTAT=ios) TALOW(1:NM)
                           IF (ios > 0) then 
                              WRITE (*, 9022) ' Error parsing min temp values from line.'
                              CALL ALSTOP(255)
                           END IF
                           taldate = HOUR_FROM_DATE(talyear, talmonth, talday, talhour, talminute)
                        END IF
