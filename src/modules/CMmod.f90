...
!----------------------------------------------------------------------*
! SUBROUTINE CMRD
! Description: Read CM (Contaminant Transport) data input file.
!----------------------------------------------------------------------*
...
!----------------------------------------------------------------------*
   SUBROUTINE CMRD(CMD, CPR, MAX_NUM_CATEGORY_TYPES, NCONEE, NELEE, NEL, NLF, NLFEE, &
      ...
      ! Automatic arrays (dimensioned via dummy arguments)
      ...
      INTEGER :: idum_scalar(1)
      LOGICAL :: LDUM(1)
      ! LOGICAL :: ISFLXB, ISADNL
      CHARACTER(LEN=80) :: CDUM(1)
      ...
      DO I = 1, NCONCM
         ...
         IF (.NOT. ISCNSV(I)) THEN
            ...
            CALL ALREDF(0, CMD, CPR, ':CM26', 1, 1, CCAPIN(I:I))
         ELSE
            ! * Initial concentration for link elements
            CALL ALREDF(0, CMD, CPR, ':CM26a', 1, 1, CCAPIN(I:I))
            ! DO J = 1, NLF
            ...
            IF (NUM_CATEGORIES_TYPES(I) > MAX_NUM_CATEGORY_TYPES .OR. NUM_CATEGORIES_TYPES(I) <= 0) THEN
            ...
            DO NC = 1, NUM_CATEGORIES_TYPES(I)
               CALL ALREDI(0, CMD, CPR, ':CM26d', 1, 1, idum_scalar)
               NTBL = idum_scalar(1)
      ...
      DO I = 1, NFEX
         ...
         ! DCOPY native array slice mapping: INCY was NELEE, effectively assigning
         ! to CCAPE(IEL, 1:NCONCM)
