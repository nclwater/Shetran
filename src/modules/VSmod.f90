...
!DOUBLEPRECISION :: VSAIJsv(4,LLEE,NELEE)
   ...
   integer,parameter :: error_counterntallowed=1000
...
CONTAINS
   ...
   SUBROUTINE VSCOLM (EESN, CWV, CWL, VSK3D, BCHELE, ELEVEL, &
      ...
      ! VSPDKR, VSPDET, FID_logfile, ERROR, error_counterntallowed, ZERO, half
      ...
      IF (NIT > NITMAX .AND. ELEVEL > 0) THEN
         ...
         IF (errorcount < error_counterntallowed) THEN
            CALL ERROR (ELEVEL, 1036, FID_logfile, IEL, 0, 'Maximum iterations in VSS column solver')
         ELSE IF (errorcount == error_counterntallowed) THEN
...
9030  FORMAT('Soil type ',I4,' not expected for soil property tables.')
   ...
   SUBROUTINE VSSIM ()
      ...
      IF (.NOT. g670) THEN
         ...
         IF (errorcount2 < error_counterntallowed) THEN
            CALL ERROR(ERRLVL_error, 1039, FID_logfile, 0, 0, 'Maximum iterations in VSS global solver')
         ELSE IF (errorcount2 == error_counterntallowed) THEN
