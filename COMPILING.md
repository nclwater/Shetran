...
The older manual Visual Studio workflow is retained here for reference only. Prefer the CMake-generated solution above for normal development, because it configures module dependencies and the HDF5 build automatically.

1. Start Visual Studio 2022. Select **Continue without code**.
2. Create a Visual Fortran empty console application with **File > New > Project**, search for Fortran, and select **Empty Project**.
...
3. In **Project > Add Existing Item...**, add the following to the project:
   ...
   * All `.f90` files from `src/visualisation`, except `include_extend_s.f90` and `include_increment.f90`, which must be present but not in the project.
   * All `.lib` files from `external/library-files`. Use **All Files (*.*)** if the files are not visible.
   * In Solution Explorer, click **Resource files**, then use **Project > Add Existing Item** to add `resource1.rc` from `src/resource`.
4. Copy the full path for the `include` directory, for example `C:\Users\sjbir_000\Documents\shetrn\external\Include`, and add it under **Project > SHETRAN Properties > Configuration Properties > Fortran > General > Additional Include Directories**.

   Depending on the selected item in Solution Explorer, this may be shown as **Properties** rather than **SHETRAN Properties**. For this and the following settings, set **Configuration** to **All Configurations** so both Debug and Release are updated.

5. In **Fortran > Optimization**, set **Heap Arrays** to `0`, which allocates temporary arrays on the heap rather than the stack.
6. In **Fortran > Libraries**, change **Runtime Library** to **Multithreaded** so the library code is included in the executable.
7. In **Fortran > Optimization**, change **Optimization** to **Maximum Speed**, corresponding to `/O2`.
8. In **Fortran > Floating Point**, change **Floating Point Model** to `fp:precise`.
9. Build the project with **Build > Build Solution**, using either Debug or Release.
