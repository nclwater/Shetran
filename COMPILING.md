...
#### Manual Project Setup (Not Recommended)

**Strongly discouraged:** Manual Visual Studio project configuration is not recommended for SHETRAN. This project requires:

* Automatic Fortran module dependency sorting (managed by CMake)
* HDF5 built from source as an external project (handled automatically by CMake)
* Correct compiler and runtime settings across multiple build configurations

These requirements are complex to maintain manually and the CMake approach (see "Generating the Solution" above) is far simpler and more reliable.

If you absolutely must set up manually, you would need to:

1. First, build HDF5 from source separately:

   * Extract `external/tarballs/hdf5-1.14.6.tar.gz` to a temporary location (e.g., `C:\temp\hdf5-1.14.6`)
   * Create a build directory: `mkdir C:\temp\hdf5-1.14.6\build`
   * Open an Intel oneAPI command prompt and configure HDF5:

     ```cmd
     cd C:\temp\hdf5-1.14.6\build
   ...
   * Note the installation path (`C:\hdf5-install` in this example) for use in the Visual Studio project

2. Then manually create a Visual Studio Fortran project and:

   * Add all `.f90` files from `src/`, `src/modules/`, `src/parameters/`, and `src/visualisation/` (excluding `include_*.f90` files)
   * Add `mod_load_filedata.f90` from `src/util/`
   * Configure **Fortran > General > Additional Include Directories** to point to your HDF5 include path (e.g., `C:\hdf5-install\include`)
   * Configure **Fortran > Libraries > Runtime Library** to "Multithreaded" (or "Multithreaded Debug" for Debug builds)
   * Link against HDF5 libraries in **Linker > Input > Additional Dependencies**: `libhdf5.lib`, `libhdf5_fortran.lib`, `libhdf5_f90cstub.lib`, `libhdf5_hl.lib`, `libhdf5_hl_fortran.lib`, `libhdf5_hl_f90cstub.lib`
   * Configure **Linker > General > Additional Library Directories** to point to your HDF5 lib path (e.g., `C:\hdf5-install\lib`)
   * Ensure module files are compiled in the correct dependency order

**This approach is error-prone.** Use CMake instead.
...
1. **CMake** (version 3.20 or higher) available on PATH.
2. A supported Fortran compiler:
...
3. Standard build tools (for example GNU Make) installed.
4. The **HDF5 1.14.6** source tarball (`hdf5-1.14.6.tar.gz`) placed in `external/tarballs/`.
