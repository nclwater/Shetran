...
The start of every output time has `time=` and each variable is named before being output. 
The code is:
...
This means the results from the simulation in dunsop-hot2 (which only starts after 504 hours) should be the same as from 504 hours in dunsop-hot1 (which runs for 1460 hours). 
Tests show that although they are very similar they are not identical.


Note that the hotstart file (e.g `input_output_dunsop_hot.txt`) can be edited. 
So for example, the output from a particular time can be selected and the time changed. 
So you could select the data from final time in the file and make a new hotstart file containing just this data and change the time=1 hour. 
Then set the hotstart to start after 0.5 hours and this data will then be used for the entire simulation. 
