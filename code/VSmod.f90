...
USE SGLOBAL
USE ALmod, ONLY : ALINIT, ALSPRD, ALREAD
!USE SGLOBAL,  ONLY : 
...
INTEGER :: NCAT, NELEM, NCOUNT, NDUM, NSP, NW  
...
! read layer data
   ...
   CALL ALREAD (6, VSD, PPPRI, ':VS08a', NELEE, NLYREE, NCAT, CDUM, &
    IVSDUM, RVSDUM)
! for NCAT=1, set all elements = category 1
   IF (NCAT.EQ.1) THEN  
...
! for > 1 category read in categories for links (if required) and grids
   ELSE  
      IF (BEXBK.AND.total_no_links.GT.0) THEN  
         CALL ALREAD (2, VSD, PPPRI, ':VS08b', total_no_links, 1, NCAT, CDUM, &
          IVSCAT, DUMMY)
      ENDIF  
      CALL ALREAD (4, VSD, PPPRI, ':VS08c', NX, NY, NCAT, CDUM, &
       IDUM, DUMMY)
      DO 300 IY = 1, NY  
         IXY0 = (IY - 1) * NX  
         DO 200 IX = 1, NX  
